...
class Person extends Component {
    addPerson=()=>{
        ...
        if(name==='' || age==='')return;
        let newPerson={id:this.props.peopleArr.length+1,name,age};
    ...
    render() {
        return (
            <div>
                ...
                <h4>当前的求和值为：{this.props.sum}</h4>
                {
                    this.props.peopleArr.map((item,index)=>{
...
export default connect(
    // 这个是redux帮忙保存的状态对象
    state=>({peopleArr:state.people,sum:state.sum}),
